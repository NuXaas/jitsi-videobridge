    ...
    if "calculated_rate" in series:
        ...
        for i in range(9):
            ...
            if encoding_quality in df_rates.columns:
                ax_bitrate.plot(
                    df_rates["time"], df_rates[encoding_quality], label=encoding_quality
                )
...
def plot(args):
    ...
    endpoint_ids = (
        [args.endpoint_id] if args.endpoint_id else df["endpoint_id"].unique()
    )
...
def check_endpoint(df, endpoint_id):
    ...
    assert not len(
        df_update[mask_1 & mask_2 & mask_3 & mask_4]
    ), "{} is not sending as much as it should.".format(endpoint_id)

    # property 2: check that the target never exceeds the ideal
    mask_5 = (df_update["total_ideal_bps"] - df_update["total_target_bps"]) < 0
    assert not len(
        df_update[mask_5]
    ), "{} has a target bitrate that exceeds the ideal bitrate.".format(endpoint_id)
...
def check(args):
    df = read_json(args.infile)
    conference_ids = (
        [args.conference_id] if args.conference_id else df["conference_id"].unique()
    )
...
if "__main__" == __name__:
    ...
    parser_plot.add_argument(
        "--series",
        nargs="+",
        choices=["did_update", "new_bwe", "sent_padding", "calculated_rate"],
